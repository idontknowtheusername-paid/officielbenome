...
const getHeroListingsByHour = async (limit = 6) => {
  ...
  try {
    ...
    const { data: premiumListings, error } = await supabase
      ...
      .limit(limit * 2); // Prendre plus pour avoir du choix

    if (error) throw error;

    // Si pas assez d'annonces premium, compléter avec des annonces populaires
    let finalListings = premiumListings || [];
    ...
    console.log(`🔄 Hero rotation: ${finalListings.length} total, ${rotatedListings.length} affichées, catégorie: ${currentCategory}, heure: ${currentHour}h`);
