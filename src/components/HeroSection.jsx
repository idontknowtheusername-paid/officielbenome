...
const HeroSection = () => {
  ...
  return (
    <section className="relative min-h-[80vh] flex items-center justify-center py-20 md:py-32 overflow-hidden">
      ...
      {/* Animated Blobs; respect user prefers-reduced-motion */}
      {shouldReduceMotion ? (
        <>
          <div
            className="absolute top-1/4 left-1/4 w-72 h-72 bg-purple-500/20 rounded-full filter blur-2xl opacity-40 -z-10"
            aria-hidden="true"
          />
          <div
            className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-blue-500/20 rounded-full filter blur-2xl opacity-40 -z-10"
            aria-hidden="true"
          />
        </>
      ) : (
      ...
      <div className="container mx-auto px-4 md:px-6 text-center relative z-10">
        ...
        >
          {/* Optimized avatar with explicit dimensions to reduce CLS */}
          <OptimizedImage
            src="https://images.unsplash.com/photo-1660527382879-14bbba78edb1"
            alt={personalData.name}
            className="rounded-full border-4 border-primary shadow-xl overflow-hidden w-32 h-32 md:w-40 md:h-40"
            context="hero"
            quality="high"
            showSkeleton={true}
            width={160} /* best-effort intrinsic size to reduce CLS (md size) */
            height={160}
          />
        </motion.div>
        ...
        >
          <Button
            asChild
            size="lg"
            className="bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white shadow-lg transform hover:scale-105 transition-transform duration-300 w-full sm:w-auto"
          >
            <Link to="/projects">
          ...
          <Button
            ...
            variant="outline"
            size="lg"
            className="border-primary text-primary hover:bg-primary/10 hover:text-primary shadow-lg transform hover:scale-105 transition-transform duration-300 w-full sm:w-auto"
          >
        ...
        >
          ...
          >
            Prêt à collaborer ? Contactez-moi{" "}
