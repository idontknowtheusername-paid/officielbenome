...
const ServicesPage = () => {
  ...
  // J'ai ajouté 'refresh' dans la déstructuration car il était utilisé plus bas mais manquant ici
  const { 
    listings, 
    loading, 
    error, 
  ...
  const handleRefresh = async () => {
    return new Promise(async (resolve) => {
      if (refresh) {
        // Si le hook useListings expose une fonction refresh, on l'utilise
        await refresh();
      } else {
        // Sinon, solution de secours fiable : recharger la page
  ...
  const pageContent = (
    ...
    <PullToRefresh onRefresh={handleRefresh} pullingContent=''>
      <div className={`min-h-screen bg-gradient-to-br from-background via-background to-purple-900/10 text-foreground ${isAppMode ? 'py-4 pb-24' : 'py-12'}`}>
          ...
          {error && (
            ...
            >
              ...
              <div className="flex gap-3">
                ...
                <Button onClick={() => refresh && refresh()} variant="outline" className="mt-4">
                  Réessayer
                </Button>
              </div>
            </motion.div>
  ...
  if (isAppMode) {
    return (
      <MobilePageLayout title="Services" showBack>
        {pageContent}
      </MobilePageLayout>
    );
  }
